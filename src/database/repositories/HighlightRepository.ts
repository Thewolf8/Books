...
export const updateHighlight = async (id: string, updates: Partial<Pick<Highlight, 'comment' | 'color' | 'x1' | 'y1' | 'x2' | 'y2'>>): Promise<void> => {
  const sets: string[] = [];
  const values: any[] = [];
  ...
  await db.execute(`UPDATE highlights SET ${sets.join(', ')} WHERE id = ?`, values);
};
...
export const getHighlightById = async (id: string): Promise<Highlight | null> => {
  const result = await db.execute('SELECT * FROM highlights WHERE id = ?', [id]);
  if (!result.rows || result.rows.length === 0) return null;
  return rowToHighlight(result.rows[0]);
};

export const getHighlightsForBook = async (bookId: string): Promise<Highlight[]> => {
...
export const getHighlightsForPage = async (bookId: string, pageNumber: number): Promise<Highlight[]> => {
  const result = await db.execute(
...
export const getAllHighlights = async (): Promise<HighlightWithBook[]> => {
  const result = await db.execute(
...
export const getFilteredHighlights = async (filters: VaultFilters): Promise<HighlightWithBook[]> => {
...
export const deleteHighlightsForBook = async (bookId: string): Promise<void> => {
